        ...
        <div class="card">
            ...
            <form action="patients" method="post"
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" required placeholder="John Doe" value="${editablePatient.name}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="doctorId" required>
                        ...
                        <c:forEach var="doc" items="${doctors}">
                            <option value="${doc.id}" ${(editablePatient !=null && editablePatient.doctor.id==doc.id)
                                ? 'selected' : '' }>${doc.name}</option>
                        </c:forEach>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="nurseId" required>
                        ...
                        <c:forEach var="nurse" items="${nurses}">
                            <option value="${nurse.id}" ${(editablePatient !=null && editablePatient.nurse.id==nurse.id)
                                ? 'selected' : '' }>${nurse.name}</option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <table>
                    ...
                    <tbody>
                        <c:forEach var="p" items="${patients}">
                            <tr>
                                ...
                                <td style="display: flex; gap: 10px;">
                                    <a href="patients?action=edit&id=${p.id}" class="btn btn-info btn-sm">Edit</a>
                        ...
                        <c:if test="${empty patients}">
                            <tr>
                                <td colspan="6"
                                    style="text-align: center; color: var(--text-secondary); padding: 40px;">No patients
                                    found.</td>
                            </tr>
                        </c:if>
                    </tbody>
